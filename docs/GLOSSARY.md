...
| **Provisional counterparty string** | A company/trust/asset named in a declaration, stored as a **first-class entity** (stable UUID: raw + normalised string + embedding) referenced by declaration events, explicitly **unresolved/provisional**. **Not clustered or resolved in v1** (similarity is computed at query time); the UUID is the anchor v2 resolution and the graph attach to. v1 never asserts a legal identity (e.g. ACN) for it. |
...
| **Publish step (publish gate)** | The one-way export that reads Postgres and emits the sanitised public artifacts (static HTML, Pagefind search index, downloadable SQLite, CSV/Parquet). The single enforcement point for **verified-only** and **minimise the person**: non-public rows are *physically absent* from the output, not merely filtered. Postgres never faces the public. |
